<?php
	...
	$DB[roles]->connect("roles", "SELECT r.* FROM (SELECT * FROM roles ORDER BY lastlogin_time DESC) AS r, online o, users u WHERE r.userid=o.Id AND u.ID=r.userid GROUP BY r.userid ORDER BY r.roleid ASC");
	$roles = $DB[roles]->retrieve();
	$DB[itemlist] = new GameDBD();
	$DB[itemlist]->connect("itemlist", "SELECT u.* FROM uwebitems i, itemlist u WHERE u.iid=i.iid GROUP BY i.iid ORDER BY i.iid ASC");
	$itemlist = $DB[itemlist]->retrieve();
	$DB[roles2] = new GameDBD();
	$DB[roles2]->connect("roles", "SELECT r.* FROM (SELECT * FROM roles ORDER BY lastlogin_time DESC) AS r, online o, users u WHERE r.userid=o.Id AND u.ID=r.userid GROUP BY r.userid ORDER BY r.name ASC");
...
<?php
	...
	$DB[items]->connect("uwebitems", "SELECT * FROM uwebitems ORDER BY isdate DESC LIMIT 0,50 ");
?>
