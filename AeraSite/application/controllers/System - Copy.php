...
class System extends CI_Controller {
	...
	public function RecentActivity()
	{
		...
		if ($this->input->get("_") > 0)
		{
			...
			$query = $this->db->query("select * from activities WHERE ((amsg not Like '%test%') AND (amsg not Like '%:LOG%') AND (amsg not Like '%quest%') AND (amsg not Like '%logged in%') AND (amsg not Like '%admin%') AND (amsg not Like 'gm%')) and ((vstr1 not Like '%test%' AND vstr1 not Like 'admin' AND vstr1 not Like 'admin%' AND vstr1 not Like 'gm%')) ORDER BY aid DESC LIMIT 0,$limit;");
			foreach ($query->result_array() as $row)
			{
				if ($row['amsg'] == "PW:LOGIN")
					...
					$feeds['text']	= $row['amsg'];
				$this->aera->addviews("content", "home.right", $feeds);
			}
		}
		$this->db->Close();
		$this->viewpage(FALSE);
	}
	public function RecentForum()
	{
		$this->set("default", "default", "htm");
		$AeraDB['www'] = $this->load->database("www", TRUE);
		...
		if ($this->input->get("_") > 0)
		{
			...
			$query = $AeraDB['www']->query("select p.*, u.username from (SELECT * FROM mybb_posts ORDER BY pid DESC) as p, mybb_users u WHERE u.uid=p.uid GROUP BY p.fid ORDER BY p.pid DESC LIMIT 0,10;");
			if ($query->num_rows() >= 1)
			{
				foreach ($query->result_array() as $row)
				{
					...
					$this->aera->addviews("content", "home.left", $prow);
				}
			}
		}
		$AeraDB['www']->Close();
	...
	public function RecentChats()
	{
		...
		if ($this->input->get("_") > 0)
		{
			$query = $AeraDB['www']->query("select c.*, r.name from chats c, roles r WHERE r.roleid=c.src AND type='Chat' ORDER BY c.cid DESC LIMIT 0,10");
			if ($query->num_rows() >= 1)
			{
				foreach ($query->result_array() as $row)
				{
					...
					else
					{
						...
						$this->aera->addviews("content", "home.poster", $prow);
					}
				}
			}
		}
	...
	public function TopForumPosters()
	{
		$this->set("default", "default", "htm");
		$AeraDB['www'] = $this->load->database("www", TRUE);
		...
		if ($this->input->get("_") > 0)
		{
			$query = $AeraDB['www']->query("SELECT u.username as name, COUNT(p.uid) as total FROM mybb_posts p, mybb_users u WHERE u.uid=p.uid GROUP BY p.uid ORDER BY COUNT(p.uid) DESC LIMIT 0,10;");
			if ($query->num_rows() >= 1)
			{
				$tpnum = 0;
				foreach ($query->result_array() as $row)
	...
	public function RecentNews()
	{
		...
		while ($i < 5)
		{
			if ($entry[$i]->author == "Aera Gaming International")
			{
				...
				$this->aera->addviews("content", "home.news", $fbxml);
				$i++;
			}
		}
		$this->viewpage(FALSE);
