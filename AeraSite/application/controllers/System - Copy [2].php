	...
	public function RefreshHome()
	{
		...
		if ($this->input->get("_") > 0)
		{
			...
			if (true)
			{
				...
				$qrecent = $AeraDB['pwi']->query("select * from activities WHERE ((amsg not Like '%test%') AND (amsg not Like '%:LOG%') AND (amsg not Like '%quest%') AND (amsg not Like '%logged in%') AND (amsg not Like '%admin%') AND (amsg not Like 'gm%')) and ((vstr1 not Like '%test%' AND vstr1 not Like 'admin' AND vstr1 not Like 'admin%' AND vstr1 not Like 'gm%')) ORDER BY aid DESC LIMIT 0,$limit;");
				$feeds = array();
			...
			{
				//COUNTRY
				$querycountry = $AeraDB['pwi']->query("SELECT ipcountry FROM users GROUP BY ipcountry ORDER BY COUNT(ID) DESC;");
				$cnum = 0;
				$acprow = array();
				if ($querycountry->num_rows() >= 1)
				{
					foreach ($querycountry->result_array() as $cprow)
					{
						...
						{
							$acprow['content']		.= '<img style="width:20px; height:auto;" src="/images/world/'.strtolower($cprow['ipcountry']).'.gif"> ';
						}
					}
			...
			{
				//COUNTRY
				$vquery = $AeraDB['pwi']->query("SELECT COUNT(vid) as vtotal, vcountry FROM visitors GROUP BY vcountry ORDER BY COUNT(VID) DESC;");
				$cnum = 0;
				$vprow = array();
				if ($vquery->num_rows() >= 1)
				{
					foreach ($vquery->result_array() as $cprow)
					{
						...
						{
							$vprow['content']		.= '<img style="width:20px; height:auto;" src="/images/world/'.strtolower($cprow['vcountry']).'.gif"> x'.$cprow[vtotal].' ';
						}
					}
